...
const ManagerEditModal = ({ isOpen, toggle, userId, fetchUsers }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/api/users/updateM/${userId}`, formData);
      fetchUsers();
      toggle();
    } catch (error) {
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle} className="modal-overlay">
      <div className="modal">
        <ModalHeader className="modal-header" toggle={toggle}>Edit Manager</ModalHeader>
        <ModalBody>
          <Form onSubmit={handleSubmit}>
            ...
            <FormGroup>
              ...
              <Input type="text" name="agence" id="agence" value={formData.agence} onChange={handleChange} />
            </FormGroup>
            <FormGroup>
              <Label for="numTel">Phone Number</Label>
              <Input type="text" name="numTel" id="numTel" value={formData.numTel} onChange={handleChange} />
            </FormGroup>
            <FormGroup>
              <Label for="buisnessEmail">Business Email</Label>
              <Input type="email" name="buisnessEmail" id="buisnessEmail" value={formData.buisnessEmail} onChange={handleChange} />
            </FormGroup>
